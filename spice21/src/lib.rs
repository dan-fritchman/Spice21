...
impl Component for Resistor {
    fn setup(&mut self, mat: &mut Matrix) {
        match (self.p, self.n) {
            (NodeRef::Num(p), NodeRef::Num(n)) => {
                self.pp = Some(mat.make(p, p));
                self.nn = Some(mat.make(n, n));
                self.pn = Some(mat.make(p, n));
                self.np = Some(mat.make(n, p));
            }, 
            (NodeRef::Num(p), NodeRef::Gnd) => {
                self.pp = Some(mat.make(p, p));
            }, 
            (NodeRef::Gnd, NodeRef::Num(n)) => {
                self.nn = Some(mat.make(n, n));
            }, 
            (NodeRef::Gnd, NodeRef::Gnd) => { }, 
        }
    }
    fn load(&self) -> Stamps {
        let mut v: Vec<(Eindex, f64)> = vec![];
        if let Some(pp) = self.pp { v.push((pp, self.g)); }
        if let Some(nn) = self.nn { v.push((nn, self.g)); }
        if let Some(pn) = self.pn { v.push((pn, -1.0*self.g)); }
        if let Some(np) = self.np { v.push((np, -1.0*self.g)); }
        return Stamps {G:v, J:vec![], b:vec![]};
    }
}
...
struct Isrc { 
 ...
 impl Component for Isrc {
    ...
    fn load(&self) -> Stamps {
        ...
        if let NodeRef::Num(pp) = self.p { b.push((pp, self.i))}
        if let NodeRef::Num(nn) = self.n { b.push((nn, -1.0 * self.i))}
        return Stamps { G:vec![], J:vec![], b:b};
    }
 }
...
impl Circuit {
    fn new() -> Circuit { 
        Circuit {
            ...
            nodes:vec![],
            vars:0,
            node0: Node { rf:NodeRef::Gnd, solve:false},
        }
    }
    fn add_node(&mut self) -> NodeRef {
        let rf = NodeRef::Num(self.nodes.len());
        let node = Node {rf:rf.clone(), solve:true};
...
impl Stamps {
    fn new() -> Stamps { Stamps{G:vec![], J:vec![], b:vec![]} }
}
...
impl DcOp {
    fn new(mut ckt:Circuit) -> DcOp {
        ...
        for mut comp in ckt.comps.iter_mut() { comp.setup(&mut mat); }
        DcOp { ckt, mat, x:vec![0.0; size], rhs:vec![0.0; size] }
    }
    fn solve(&mut self) -> SpResult<Vec<f64>> {
        ...
        for k in 0..100 { // FIXME: number of iterations
            ...
            for comp in self.ckt.comps.iter() {
                ...
                for upd in updates.G.iter() { self.mat.update(upd.0, upd.1); }
                for upd in updates.b.iter() { self.rhs[upd.0] += upd.1; }
            ...
            // Check convergence
            if self.converged(&dx, &res) { return Ok(self.x.clone()); }
            // Didn't converge, add Jacobian terms
            for upd in jupdates.iter() { self.mat.update(upd.0, upd.1); }
            // Solve for our update
            let dx = self.mat.solve(res)?;
            // And update our guess 
            for r in 0..self.x.len() { self.x[r] += dx[r]; }
        }        
...
mod tests {
    ...
    fn test_add_res() -> TestResult {
        let mut c = Circuit::new();
        let rf = c.add_node();
        let r = Resistor { g:1e-3, p:rf, n:NodeRef::Gnd, 
            pp:None, nn:None, np:None, pn:None};
        c.add_comp(r);
        Ok(())
    }
    ...
    fn test_dcop2() -> TestResult {
        ...
        c.add_comp(r);
        let i = Isrc { i: 1e-3, p: rf, n:NodeRef::Gnd};
        c.add_comp(i);
    ...
    fn test_dcop3() -> TestResult {
        ...
        c.add_comp(r2);
        let i = Isrc { i: 1e-3, p: n2, n:NodeRef::Gnd};
        c.add_comp(i);
